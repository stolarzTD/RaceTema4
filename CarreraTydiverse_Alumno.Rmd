...
cran %>%
  mutate(size_mb = size / 2^20) %>%
  select(ip_id, country, package, size, size_mb) %>%
  print()
...
cran %>%
  ...
  filter(total_size > quantile(total_size, probs = 0.9)) %>%
  arrange(desc(total_size)) %>%
  ggplot(aes(x = reorder(country, total_size), y = total_size)) +
  geom_point() +
  xlab("Country") +
  ylab("Total Size of Downloads") +
  ggtitle("Top Countries by Download Size") +
  theme_minimal()
...
ggplot(sat_grouped, aes(x = total, y = prop, fill = sex)) +
...
nyc <- with_tz(now(), "America/New_York")
salida <- nyc + days(2)
salida <- update(salida, hour = 17, minute = 34)

arrival_ny <- salida + hours(15) + minutes(50)

arrival_hk <- with_tz(arrival_ny, "Asia/Hong_Kong")

last_time_sg <- with_tz(ymd("2008-06-17"), "Asia/Singapore")
time_since_last <- as.period(interval(last_time_sg, arrival_hk))
